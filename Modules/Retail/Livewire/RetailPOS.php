...
namespace Modules\Retail\Livewire;
...
class RetailPOS extends Component
{
    ...
    public function scanBarcode()
    {
        ...
        if ($item) {
            ...
            session()->flash('error', 'Item not found with barcode: ' . $this->barcodeInput);
        }
    ...
    public function processPayment()
    {
        if ($this->paymentMethod === 'cash' && $this->amountReceived < $this->total) {
            session()->flash('error', 'Insufficient amount received!');
            return;
        }
    ...
    {
        return view('retail::livewire.retail-pos')
            ->layout('layouts.app');
    }
}
